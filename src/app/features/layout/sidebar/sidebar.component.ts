...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  filteredNavGroups = signal<NavGroup[]>([]);
      ...
      this.authService.currentUser$.subscribe((user) => {
        ...
        if (user) {
          this.productsService.getLowStock().subscribe({
            next: (list) => {
              this.outOfStockCount.set(list.filter((p) => p.stock === 0).length);
              this.lowStockCount.set(list.filter((p) => p.stock > 0).length);
            },
            error: () => {
              this.outOfStockCount.set(0);
  ...
  avatarColorClass = computed(() =>
    this.currentUser()?.role === 'admin' ? 'bg-emerald-600' : 'bg-indigo-600',
  );
