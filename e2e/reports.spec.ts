...
test.describe('Módulo de Reportes', () => {
  ...
  test('RE-01: carga la pantalla de reportes', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Reportes' })).toBeVisible({ timeout: 8000 });
  });

  // RE-02
  test('RE-02: muestra el resumen ejecutivo con métricas', async ({ page }) => {
    // Debe mostrar algún dato de resumen
    const hasMetrics =
  ...
  // RE-03
  test('RE-03: los selectores de rango de fechas están presentes', async ({ page }) => {
  ...
  // RE-04
  test('RE-04: el filtro de fecha "hoy" carga datos del día actual', async ({ page }) => {
  ...
  // RE-06
  test('RE-06: muestra el gráfico de evolución de ingresos', async ({ page }) => {
    // Canvas o SVG del gráfico de evolución
    const hasChart =
  ...
  // RE-07
  test('RE-07: muestra la distribución de métodos de pago', async ({ page }) => {
  ...
  // RE-08
  test('RE-08: muestra el ranking de productos más vendidos', async ({ page }) => {
    const hasRanking =
  ...
  // RE-10
  test('RE-10: cambiar el rango de fechas dispara una nueva petición a la API', async ({ page }) => {
