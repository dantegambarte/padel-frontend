...
test.describe('Autenticación', () => {
  ...
  test('debe mostrar el formulario de login', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: 'La Caldera' }),
    ).toBeVisible();
    await expect(
      page.getByText('Sistema de Gestión de Canchas de Padel'),
    ).toBeVisible();
  ...
  test('el botón debe estar deshabilitado con campos vacíos', async ({
    page,
  }) => {
    await expect(
  ...
  test('login exitoso como Administrador redirige al dashboard', async ({
    page,
  }) => {
    test.slow();
    await page.getByRole('textbox', { name: 'Usuario' }).fill('admin');
  ...
  test('login exitoso como Empleado redirige al dashboard', async ({
    page,
  }) => {
    test.slow();
